...
def process_data(data, qos_parameter):
    ...
    for entry in data:
        ...
        tcp_buffer = extract_value(parameters['tcp_buffer_size'])
        qos_value = extract_value(qos[qos_parameter])

        if tcp_buffer is not None and qos_value is not None:
            tcp_buffers.append(tcp_buffer)
            qos_values.append(qos_value)

    return tcp_buffers, qos_values
...
if __name__ == "__main__":
    file_path = "ATCN-Program-tcpBufferSize.txt"
    data = read_data(file_path)
    #print(data)
    for qos_parameter in ['Throughput', 'Transmission Delay', 'Packet Loss Rate', 'Jitter']:
        tcp_buffers, qos_values = process_data(data, qos_parameter)
        unit = get_unit(qos_parameter)  # Implement get_unit function based on your requirements
        plot_filename = f'{qos_parameter}_vs_TCP_Buffer_Size.png'
        plot_figure(tcp_buffers, qos_values, 'TCP Buffer Size', f'{qos_parameter} ({unit})', f'{qos_parameter} vs. TCP Buffer Size', plot_filename)