...
def process_data(data, qos_parameter):
    ...
    for entry in data:
        ...
        udp_rate = extract_value(parameters['UDP_rate'])
        qos_value = extract_value(qos[qos_parameter])

        if udp_rate is not None and qos_value is not None:
            udp_rates.append(udp_rate)
            qos_values.append(qos_value)

    return udp_rates, qos_values
...
if __name__ == "__main__":
    file_path = "ATCN-Program-Udprate.txt"
